...
def show_heart_disease_prediction():
    ...
    if st.sidebar.button("Prediksi"):
        ...
        anemia_num = 1 if anemia == "Ya" else 0
        diabetes_num = 1 if diabetes == "Ya" else 0
        tekanan_darah_tinggi_num = 1 if tekanan_darah_tinggi == "Ya" else 0
        kebiasaan_merokok_num = 1 if kebiasaan_merokok == "Ya" else 0

        # Konversi input menjadi array numpy
        data = np.array([[usia, jenis_kelamin_num, anemia_num, creatinine_phosphokinase, diabetes_num, pecahan_injeksi, tekanan_darah_tinggi_num, trombosit, kebiasaan_merokok_num, waktu, serum_kreatinin, serum_sodium]])
        ...
        st.write("### Nilai-nilai yang diinputkan:")
        ...
        if prediction[0] == 0:
            st.write("### Prediksi: Resiko Penyakit Jantung: Rendah")
        else:
            st.write("### Prediksi: Resiko Penyakit Jantung: Tinggi")
        st.markdown("</div>", unsafe_allow_html=True)
...
def show_healthy_lifestyle():
    ...
    st.image('TIPS HIDUP SEHAT.png', caption='hidup sehat', use_column_width=True)
    st.write("""
    ...
    st.image('olahraga.jpg', caption='olahraga', use_column_width=True)
    st.write("""
...
def show_login():
    ...
    st.write("Belum punya akun? Daftar di bawah ini.")
